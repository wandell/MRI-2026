%% Diffusion ellipsoids (tensor imaging)
% The basic ideas of diffusion tensor imaging are connected to the visualization 
% of ellipses.  We calculate and plot some key images for the diffusion imaging 
% models.  The goal is to give you some intuitions.
% 
% 
% 
% The tensor equation for the apparent diffusion coefficient.
% 
% $$\textrm{ADC}=b^{\prime } \textrm{Qb}$$
% 
% A representation of the vectors that create the ellipsoidal representation 
% of Q
% 
% $$1=u^{\prime } \textrm{Qu}$$
% 
...
% The three axes of an ellipse can each have their own length.  We set up an 
% ellipse with two axes of length 1 and one axis of length 3. 
...
[px, py, pz] = ellipsoid(center(1),center(2),center(3),ev(1),ev(2),ev(3),nFaces);

% The ellipsoid points are px,py,pz.  Don't ask me why the order is
% changed to x,z,y.
surf(px,pz,py,'EdgeAlpha',0.2,'EdgeColor',[1 1 1],'FaceColor',[0.7 0.3 1]);

% Light the surface and show it
...
% By using 6 directions, we guarantee that the tensor will perfectly fit the 
% data. (But the code below will work for any number of directions.)
...
% 
% Using the diffusion tensor to transform the unit sphere coords produces an 
% ellipsoid, even if there are non-zero off-diagonals in D.
...
%% 
% Using the eigenvalue decomposition also produces an ellipsoid. This ellipsoid 
% DOES NOT represent the predicted ADC profile. Rather, it represents the RMS 
% diffusion distance of spins that began at the center of the voxel. Note that 
% this surface will accurately predict the ADC values that happen to fall exactly 
% on one of the three axes of the ellipsoid. (They'd better, since the eigenvalues 
% are taken as the ADC's along the three axes of the ellipsoid.) This surface 
% does not, however, predict the ADC profile.
