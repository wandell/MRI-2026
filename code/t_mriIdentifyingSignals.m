...
% This tutorial introduces the student to considerations faced by fMRI investigators 
% in distinguishing signals, which we want to measure, from noise, which we don't 
% particularly care to measure. The noise can come from a variety of different 
% areas, which will be discussed in lecture. Among the topics that are covered 
% are:
%% 
...
%% 
% We use data taken from an fMRI scanning session. The session consists of several 
% scans used to localize regions of interest involved in different aspects of 
% vision.
% 
% For this week, we're only going to consider a small piece of this data: the 
% time series taken from several voxels located in the subject's right area MT. 
% Area MT (also known as V5 ) is a visual area that is highly responsive to moving 
% stimuli. 
% 
% The scan from which the data are taken is an MT localizer scan, in which the 
% subject saw alternating periods of moving and static stimuli. The stimulus was  
% a series of concentric circles. For 16 seconds, the circles would stay still, 
% then for the next 16 seconds they would move rapidly in and out, then be static, 
% then move, etc.
% 
% Each 16-second period is referred to as a *block*. Because there are only 
% two types of blocks in this scan, which alternate, this design is sometimes 
% referred to as an ABAB block design. Each pair of two blocks is called a cycle. 
% The scan consisted of six cycles, each 32 seconds long, for a total of 192 seconds.
% 
% ('Cycle' refers to the idea that, after a 32-second  cycle the stimulus is 
% in the same state as at the beginning of the cycle. We're interested in finding 
% brain regions whose activation also returns to the same activity level at the 
% same point in a cycle. But this is an aside which we'll explore later.)
%% 
% * *Class*:     Psych 204A
...
%% 
% What do you see? First, you may wonder what the y axis represents in this 
% plot. This data is taken straight from the P*.7.mag files that are created at 
% the scanner, after reconstructing the frequency- and phase-encoded signals into 
% an MR image (review the MR Imaging and BasicMR tutorials if you're confused 
% about this). So, these values represent rather arbitrary units, which depend 
% on many different factors that are subject to change from session to session, 
% and even from voxel to voxel. To get a sense of how the raw signal can vary, 
% let's plot all 21 voxels in this sample data:
...
% Signal drift
% You may have noted,  maybe before anything else, that the signal cycles up 
% and down six times. We're slowly getting to that. But first, you may notice 
% something else that may be  troubling: the signal 'droops' towards the end of 
% the scan. This change over time is known as *signal drift*, and is caused by 
% the scanner hardware, rather than the brain itself. 
% 
% Therefore, it's something to take into account when identifying signals.
% 
% There are many strategies to deal with signal drift. We'll talk about a few. 
% First, there's a linear trend removal: we assume that the drift can be described 
% by a simple line, and subtract our estimate of that drift. 
...
title('High-pass Trend removal')
%% Question 1
% (a) Where did the fourier transform change as a result of the trend removal 
% and why?
% 
% (b) Why would this help signal drift, but not, e.g. pulse / respiratory artifacts?
% 
% (c) Why, for a 192-second scan, would six cycles, but not three cycles or 
% forty cycles, be useful for experimental design? Can you think of any potential 
% drawbacks?
% 
% *(Extra Credit)* Try using detrendTSeries to perform quadratic detrending 
% on the data (assume the trend is a quadratic function), and compare it to the 
% other two types of detrending.
%% Contrast-to-Noise Ratios (CNR)
% Now we move on to the periodicities we've noted in the data. We note that 
% the main fluctuation occurs six times over the course, the same as the number 
% of times the stimulus varied between static and moving. We believe this fluctuation 
% represents the motion-selective signal from MT.
% 
% However, you may recall generating predicted hemodynamic responses during 
% the linear systems tutorial -- those predicted responses were much cleaner than 
% what we see here. There's a certain amount of noise. This can come from many 
% sources, including both the instrumentation and the brain itself, which we discuss 
% more fully in class.
% 
% Before we can come up with plans of dealing with this noise, we first want 
% a quantitative way to describe how much noise there is relative to the signal 
% we want to measure. We'll use the well-known measurement of signal-to-noise 
% ratio, or in this case, contrast-to-noise ratio (CNR). (Note that the Huettel 
% book calls this quantity functional SNR; it's also referred to as functional 
% CRN or dynamic CNR).
% 
% As its name suggests, CNR is a ratio, measuring the size of the signal we 
% measure relative to the variance in its measurement. 
...
% CNR would be higher.)
%% Simulations
% To get a sense of how noise affects the signal, let's take some fake data 
% and add varying levels of noise to it. 
...
%% 
% Now we convolve to get an estimate of the ideal response to the stimulus, 
% with no noise, and calculate the CNR (we'll use the calcCNR function, which 
% does the same calculation we did above):
...
%% 
% (Note that the zero point for this data represents the mean value during the 
% baseline period, rather than the overall mean, as was discussed earlier. Always 
% remember to check what the experiments set as the zero point when reading fMRI 
% studies!)
...
%% QUESTION 2
% (a) In the 4th subplot, you'll note that the CNR, as expressed in decibels, 
% is negative. What does this signify?
% 
% (b) What is the difference between this CNR measure, and signal-to-hoise as 
% described in the Huettel book?
%% Averaging, spatially and temporally, to enhance CNR
% Now we can describe how well we've identified a signal relative to noise -- 
% now how can we reduce the noise? 
% 
% There are many different approaches being taken for this. We'll just describe 
% one that is commonly used -- averaging the data, both spatially across voxels 
% and temporally across scans, to achieve a higher contrast-to-noise ratio.
% 
% First, let's average across voxels. We've already looked at the differences 
% across time courses from different voxels in our sample data. Let's look at 
% them again, separately for each voxel, for the first ten voxels. For each time 
% course we'll calculate the CNR for that voxel alone:
...
%% 
% The *temporal averaging* can be done across different scans of the same type, 
% which unfortunately isn't available for this data set. But we can view the repeated 
% cycles as being the same idea -- increasing the number of observations, so as 
% to improve our estimate of the signal relative to the noise.
%% QUESTION 3
% (a) Think of an instance in which spatial averaging would cause a reduction 
% in the CNR. 
% 
% (b) Think of an instance in which temporal averaging would cause a reduction 
% in the CNR. 
% 
% 
% 
