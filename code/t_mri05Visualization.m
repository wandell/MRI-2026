...
whos
%% 
% The variable anat contains the image data. We'll examine the brain mask and 
% mesh later. The other array (voxDim) tells us the size of physical space (in 
% millimeters) that each image voxel represents:

voxDim
% *Question 1*
%% 
% * What is the field of view of this image?
% * What is the volume of one voxel?
%% Examine a slice from this dataset
% The T1-weighted MR data are stored in a 3d array called 'anat'. Here's one 
% slice from the middle of this array:
...
colorbar;
%% 
% In this T1-weighted image, tissues with long T1 values show up as dark (cerebro-spinal 
% fluid, CSF), medium T1 values are gray (gray matter, GM, and muscle), and shorter 
% T1 values are white (white matter, WM, % subcutaneous fat in the scalp). In 
% addition to T1-weighting, this image includes some proton density weighting. 
% That's why bone and air, which have relatively little free water, also show 
% up as dark. Because this image is not based on a quantitative MR method, the 
% image intensity values are in arbitrary units, so only the realtive intensities 
% (image contrast) are important.
% *Question 2*
...
% Remember that typically MRI data are a 3d dataset (volume). In some cases, 
% we examine the data as individual slices as a simple movie.
...
%% 
% We can also show the middle slice of each of the three axes

figure(3);
middleSlices = round(size(anat)/2)

subplot(1,3,1); imagesc(anat(:,:,middleSlices(1)));
axis equal tight; colormap gray;

subplot(1,3,2); imagesc(squeeze(anat(:,middleSlices(2),:)));
axis equal tight; colormap gray; 

subplot(1,3,3); imagesc(squeeze(anat(middleSlices(3),:,:)));
axis equal tight; colormap gray;
%% Segment the brain tissue
% The first step to segmenting the interesting brain tissues is to remove the 
% annoying scalp and skull. This is often called "brain extraction." It can be 
% done manually, by carfully labeling each voxel. There are also some very good 
% algorithms that can do this task automatically. Outside of this script we applied 
% one of these tools (FSL BET: <http://www.fmrib.ox.ac.uk/fsl/bet2/ http://www.fmrib.ox.ac.uk/fsl/bet2/>) 
% and saved the resulting brain mask. That tool is also embedded in the CNI's 
% Flywheel data management tool.  *I should make a video showing how it is applied!!!!* 
% 
% Here is the brain mask for the same middle slice that we saw above. This 'mask' 
% is a simple image with 1's indicating brain tissue and 0's indicating non-brian 
% tissue.
...
% Question 3
%% 
% * What is the volume of the brain in cubic centimeters? Can you calculate 
% it?
...
% Question 4
%% 
% * How many peaks are there in this histogram?
% * What tissue types do these peaks represent?
%% 
...
set(p, 'FaceColor', [.5 .5 .5], 'EdgeColor',[.2 .2 .2]);
daspect([1 1 1]);
...
% Question 6
%% 
% * What is the surface area of this person's white matter, in square centimeters?
% * How many 8.5 x 11 inch sheets of paper would it take to cover this white 
% matter surface? (Hint: One inch equals 2.54 centimeters.) 
% * Assuming the cortex is 3mm thick, estimate the volume of the cortical gray 
% matter and compare this to the white matter volume.
%% Visualize the 3d white matter surface.
% First, we center the vertices on 0,0,0 so that all rotations are about the 
% center of mass: 
