...
%% 
% Quantize space into 100 regions and use meshgrid to lay these out into 2d 
% arrays that be used to compute a 100x100 image:
...
% Tesla per micrometer (that's 50 mT/m, a typical gradient strengh for clinical 
% MR scanners).
...
%% 
% Calculate the relative field strength at each spatial location for our across 
% our simulated voxel. Gradient strengths are symmetric about the center. To understand 
% the following expression, work through the units:
% 
% (micrometers * T/um + micrometers * T/um) leaves us with T. 
% 
% We scale this
% 
...
% strengths are expressed as T/cm and are symmetric about the center of the voxelSize. 
% To understand the following expression, work through the units: 
% 
% (centimeters * T/cm + centimeters * T/cm) * Hz/Tesla + Tesla*Hz/Tesla 
% 
% leaves us with MHz
...
%% 
% Note that including the B0 field in this equation simply adds an offset to 
% the spin frequency. For most purposes, we usually only care about the spin frequency 
% relative to the B0 frequency (i.e., the rotating frame of reference), so we 
% can leave that last term off and compute relative frequencies:
...
Mxy0 = 2;
curSpinPhase = zeros(size(spinCenter));
T2 = 0.10;
t = 0;
nTimeSteps = 100;
for(ti=1:nTimeSteps)
