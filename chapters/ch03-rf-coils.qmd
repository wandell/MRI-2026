...
<!-- TODO (Brian): images/ch03/rf-surface-coil-circuit.png does exist, so the commented-out figure above can simply be uncommented if you want it. Decide whether to restore it or delete the block. (The one unused file in the folder, rf-inductor-capacitor.png, is a screenshot of the capacitor-vs-inductor comparison already transcribed as the table in the callout below, so it is correctly unused. There is also a stray rf-inductor-capacitor.pdf in the images folder that nothing uses.) -->
