...
As in @fig-ch07-the-net-magnetization-is-summarized-by-two-compone, we separate the net magnetization into a component along the z-axis (longitudinal) and a component in the x-y plane (transverse). T1 describes how the longitudinal component returns to equilibrium after an RF pulse. T2, which is boxed in this slide, describes how the transverse component returns to equilibirum.
...
A receive coil sits at one position and senses only the projection of that vector onto its own axis (@fig-ch06-the-receive-coil-measures-the-electrical-radio-fre-2). As the transverse magnetization precesses around the z-axis, the projection points first toward the coil (positive) and then away from it (negative).  The measured signal oscillates at the Larmor frequency as the vector rotates. The *amplitude* of that oscillation decays over time.  The time constant of the day of the transverse vector is T2.
...
![T1-weighted images: gray is darker than white.](images/ch08/t1-weighted-images-gray-is-darker-than-white.png){#fig-ch08-t1-weighted-images-gray-is-darker-than-white width="75%"}
...
![T2-weighted images: white is darker than gray.](images/ch08/t2-weighted-images-white-is-darker-than-gray.png){#fig-ch08-t2-weighted-images-white-is-darker-than-gray width="75%"}
