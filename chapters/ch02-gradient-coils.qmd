...
Ørsted demonstrated the connection between electric current and magnetism. Gauss developed mathematical foundations for describing magnetic fields. Tesla applied these principles in practical electromagnetic devices.

The connection between electricity and magnetism was first demonstrated by the Danish physicist Hans Christian Ørsted in 1820, when he showed that a current-carrying wire deflected a compass needle placed beneath it.
...
![A multiple-turn solenoidal winding, with a Helmholtz pair shown for contrast.](images/ch02/magnetic-field-units-table.png){#fig-magnetic-field-units-table fig-alt="Photograph of a multiple-turn cylindrical coil, with a small Helmholtz-pair diagram shown below it."}

To produce a uniform field over a large volume, an MRI main magnet uses multiple closely spaced, coaxial solenoidal windings. Their positions and currents determine the field strength and homogeneity. A simple resistive solenoid would dissipate enormous power at MRI field strengths, which is why most clinical and research systems use superconducting windings.
...
For a simple solenoid, adding turns or increasing current increases the field. In an MRI magnet, the winding geometry is optimized to make $B_0$ highly uniform throughout the imaging volume.
...
These images were taken at the time of the Ultra-High Performance (UHP) gradient upgrade at the CNI: the new gradient coil arrived and the existing coil was removed and then replaced. The installation required precise mechanical alignment within the magnet bore, as well as careful attention to the strong static field.
